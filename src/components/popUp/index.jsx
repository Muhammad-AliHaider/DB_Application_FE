import { AddDesert, AddOrder, AddPerson, UpdateDesert, UpdateOrder, UpdatePerson } from "../../dataController/apiFetching/apis.jsx";
...
const PopUpForm = (props) =>{
    ...
    async function SubmitData(){
        ...
        if(props.tabActive === 0){
            const res = UpdateDesert(props.FieldID,props.Field1,props.Field2,props.Field3,props.Field4,props.Field5);
            if(res){
                props.handleClickOpen();
                alert("Desert Updated")
                props.setter((prev) => !prev)
            }
            else{
                alert("Desert Not Updated")
            }
        }
            ...
            if(props.tabActive === 0){
                const res = AddDesert(props.Field1,props.Field2,props.Field3,props.Field4,props.Field5);
            if(res){
                props.handleClickOpen();
                alert("New Desert Added")
                props.setter((prev) => !prev)
            }
            else{
                alert("Desert Not Added")
            }
            }
            else if(props.tabActive === 1){
